...
#
# ------------------------------------------------------------------------
class FamilyCard(PrimaryCard):
    """
    The FamilyCard exposes some of the basic information about a Couple.
    """
    ...
    def __init__(
        ...
        if "active" in groptions.option_space:
            ...
            anchor = "group.family"
        self.compact = grstate.config.get("%s.compact-mode" % anchor)
        if groptions.force_compact:
            self.compact = True
        self.partner1 = Gtk.HBox(hexpand=True)
        self.partner2 = Gtk.HBox(hexpand=True)
        PrimaryCard.__init__(self, grstate, groptions, family)
        self.divorced = False
        self.family = family
    ...
    def __add_couple_facts(self, anchor, family):
        ...
        option_prefix = "%s.lfield-" % anchor
        self.load_fields("facts", option_prefix, event_cache)
        if "active" in self.groptions.option_space:
            option_prefix = "%s.mfield-" % anchor
    ...
    def load_fields(self, grid_key, option_prefix, event_cache):
        ...
        key = "%sskip-marriage-alternates" % option_prefix
        args.update({"skip_marriage_alternates": self.get_option(key)})
        key = "%sskip-divorce-alternates" % option_prefix
    ...
    def load_attributes(self):
        ...
        args = {
            "skip_labels": not self.get_option("%sshow-labels" % option_prefix)
        }
    ...
    def _get_profile(self, person):
        if person:
            ...
            profile = PersonCard(self.grstate, self.groptions, person)
            return profile
        return None
