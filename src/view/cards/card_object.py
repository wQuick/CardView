...
class ObjectCard(CardView):
    ...
    def get_title(self):
        """
        Generate a title describing the card object.
        """
        if not self.primary.has_handle:
            title = self.primary.obj_lang
            if self.secondary:
                title = "%s: %s" % (title, self.secondary.obj_lang)
            return title
        ...
        if self.reference:
            return "%s: %s" % (title, self.reference.obj_lang)
        if self.secondary:
            return "%s: %s" % (title, self.secondary.obj_lang)
        return title
